...
int main(int argc, char *argv[]) {
    if (argc < 2 || argc > 4) {  // Modified to allow up to 4 arguments (program name + 3 args)
        printf("Usage: %s program.txt [commands.txt] [debug_output_file.txt]\n", argv[0]);
        return 1;
    }
    ...
    // Open debug file if provided (third argument)
    if (argc >= 4) {
        debug_fp = fopen(argv[3], "w");  // Open with "w" mode to clear the file for each new run
        if (debug_fp == NULL) {
            printf("Warning: Could not open debug file %s\n", argv[3]);
        }
    }

    const char *command_filename = (argc >= 3) ? argv[2] : NULL;
