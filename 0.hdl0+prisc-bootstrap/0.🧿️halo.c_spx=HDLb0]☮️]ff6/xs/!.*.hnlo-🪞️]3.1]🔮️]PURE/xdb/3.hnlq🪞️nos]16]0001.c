...
int setup_vmem(){
 ...
 int i;
    for (i = 0; i < BITESIZE; i++) {
      ...
      dram_yr_0[i] = -2 ; // 46 = "." (we dont need ascii yet i guess w/e)
...
printf("print_dram: \n");
 int i;
    for (i = 0; i < BITESIZE; i++) {
        printf("%c", dram_yr_0[i] + 48); // y 48? ascii
...
// can exceed halo.s , load dram is good, 
// if its "mb/gb,s u can mem map , even do page-chunks on real .txt etc...
// ofc were doing "og-basics" first...
}
...
//
if(int_reg_a == 3){
 int_reg_a =  dram_yr_0[clk_count_0]  ;
//dram_yr_0[clk_count_0] = -2 ;// clear? cap? A1/2= dontthink so
if (int_reg_a == -2){
int_reg_a = 3;
}
...
if(int_reg_b == 3){
 int_reg_b = dram_yr_0[clk_count_0] ;
//dram_yr_0[clk_count_0] = -2 ; //
...
//silenced feb3-25
//printf();
dram_yr_0[clk_count_0] = int_reg_a; //addded fb3-25

}

int post_to_cli(){
//ur supposed to print a "cli array"
//cli_0[clk_count_0] = dram_yr_0[clk_count_0] ;// int_temp_e ;
...
post_to_dram(); // scoop to mem
print_dram(); // for debug w/e (dram can be 64 bits btw)
...
setup_vmem(); //may7. moved this 2 get dram right. no clue wut it is...
